...
// ]
  return (
    ...
    <div className="shelfbook">
      <h1>Book List</h1>
      <div> {books.map(book => <BookComponent book={book}/>)}</div> 
    </div>
    </>
    )
}
...
function BookComponent({book}) {
    ...
    return (
      <div className="eachBook" style={{ padding: "70px" }}>
          ...
          <div className="book_image"><img alt="" src={book.coverImageUrl} onError={(e) => e.target.src = "not_img.png"} img style={{ height: "250px", float: "right" }}/></div>
          <p style={{ clear: "right" }}>
          ...
          <br></br>
          <div className='short_description'>{book.shortDescription}</div>
          
          <div> 
          <br></br>
          {expanded
              ? <button onClick={() => setExpanded( !expanded)}>Less about this</button>
              : <button onClick={() => setExpanded( !expanded)}>More about this</button>}
          </div>
      {/* logical AND (&&) for a set of boolean */}
            {expanded &&
            <div>
                <div className='detailed_description'><i><strong>{book.detailedDescription}</strong></i></div>
                <div className='xtrainfo'>
                {book.url &&<div><strong>URL: </strong><a href={book.url} style={{ color: "yellow" }}>{book.url}</a> </div>}
                {book.publisher &&<div><strong>Publisher: {book.publisher}</strong></div>}
                {book.publicationDate &&<div><strong>Publication Date: {book.publicationDate}</strong></div>}</div>
            </div>}
        </div>
